...
public class StudentController {
	...
	public ModelAndView addStud(StudentRegistration sreg)
	{
		ModelAndView mv = new ModelAndView("addStudent");	
		...
		//Find Data Using ID
		Optional<StudentRegistration> sreg1 = srepo.findById(sreg.getSno());
	...
	public ModelAndView updateView(StudentRegistration sreg)
	{
		...
		mv.addObject("SGender",sreg.getGender());
		mv.addObject("SAge",sreg.getAge());
		mv.addObject("SContactDetails",sreg.getContactDetails());
		mv.addObject("SDOB",sreg.getDob());
		mv.addObject("SDOJ",sreg.getDoj());
		mv.addObject("SCoursesJoined",sreg.getCoursesJoined());
	...
	public ModelAndView update(StudentRegistration sreg)
	{
		...
		stupdate.setSno(stno);
		stupdate.setName(sreg.getName());
		stupdate.setGender(sreg.getGender());
		stupdate.setAge(sreg.getAge());
		stupdate.setContactDetails(sreg.getContactDetails());
		stupdate.setDob(sreg.getDob());
		stupdate.setDoj(sreg.getDoj());
		stupdate.setCoursesJoined(sreg.getCoursesJoined());
		...
		List<StudentRegistration> s1 = srepo.findAll();
			...
			else
			{
				...
				for (StudentRegistration student : s1) 
				{
					...
					+"<td>"+student.getCoursesJoined()+"</td>"
					+"</tr>";
